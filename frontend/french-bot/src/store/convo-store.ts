...
const { state } = createStore<{ conversations: ConversationSummary[]; activeConversation: string | null }>({
  conversations: [],
...
export async function getConversations(): Promise<ConversationSummary[]> {
  ...
  state.conversations = (await res.json()).map((convo: { id: string; name: string; timestamp: string }) => ({
    ...convo,
...
export async function getConversation(conservationId: string): Promise<Conversation> {
  ...
  const conversation: Message[] = (await res.json()).map((message: { id: string; message: string; is_user: boolean; timestamp: string }): Message => {
...
export async function setActiveConversation(convoId: string) {
